...
with tab1:
    ...
    # --- MUDANÇA VISUAL: GRAFICOS DE PIZZA (DONUT) PARA MOBILE ---
    st.markdown("---")
    ...
    with col_abc_forn:
        ...
        # Adiciona info ao passar o mouse
    ...
    with col_abc_mat:
        ...
        fig_pie_m.update_traces(textinfo='percent', textposition='inside') # No material, label as vezes é grande, melhor só %
...
with tab3:
    ...
    if item_bid_display:
        ...
        m1.metric("Melhor Preço", format_brl_str(df_item['v_unit'].min()))
        m2.metric("Pior Preço", format_brl_str(df_item['v_unit'].max()))
        m3.metric("Média", format_brl_str(df_item['v_unit'].mean()))
...
with tab4:
    ...
    termo = c_busca.text_input("Buscar Item:", placeholder="Ex: Cimento...")
    cat_sel = c_cat.multiselect("Família:", df_final['Categoria'].unique())
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Ultima_Data": st.column_config.DateColumn("Última", format="DD/MM/YYYY")
        },
